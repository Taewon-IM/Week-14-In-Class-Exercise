...
public:
    ...
    virtual float shootingSuccessRate(int base_skill) {
        // Formula: 0.5 * base_skill * (1 - fatigue_level)
        return 0.5f * (float)base_skill * (1.0f - fatigue_level);
    }

    virtual float tackleSuccessRate(int base_skill) {
        // Formula: base_skill * (1.0f - (0.5f * fatigue_level))
        return (float)base_skill * (1.0f - (0.5f * fatigue_level));
    }
    ...
    void checkEnergy(int base_stamina) {
        int effective_stamina = (int)(base_stamina * (1.0f - fatigue_level));
...
int main() {
     ...
     */
    Player* defender = new Defender("Minjae Kim");
     ...
     */
    Player* striker = new Striker("Heungmin Son");
